...
def render_annotation(
    ...
    for cam in cams:
        _, boxes, _ = nusc.get_sample_data(
            sample_record["data"][cam], box_vis_level=box_vis_level, selected_anntokens=[anntoken]
        )
        if len(boxes) > 0:
    ...
    if extra_info:
        ...
        information = " \n".join(
            [
                f"category: {category}",
                "",
                f"# lidar points: {lidar_points:>4}",
                f"# radar points: {radar_points:>4}",
                "",
...
def get_sample_data(
    ...
    """Returns the data path as well as all annotations related to that sample_data. Note that the
...
def get_color(category_name: str):
    ...
    This method works for the general nuScenes categories, as well as the nuScenes detection
    categories.
    """
    a = [
        "noise",
        ...
        if sensor_modality in ["lidar", "radar"]:
            ...
            if with_anns:
                ...
                for box in boxes_gt:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(
                        ax[j + 2, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c)
                    )
...
if __name__ == "__main__":
    nusc = NuScenes(
        ...
        dataroot="/home/can/Desktop/research/sensorfusion/data/nuscenes/full",
        verbose=True,
    )
    # render_annotation('7603b030b42a4b1caa8c443ccc1a7d52')
    # bevformer_results = mmcv.load('/home/can/Desktop/research/sensorfusion/test/bevformer_small/BEVsmall_rain/pts_bbox/results_nusc.json')
    bevformer_results = mmcv.load(
        "/home/can/Desktop/research/sensorfusion/test/bevformer_small/RED_rain/pts_bbox/results_nusc.json"
    )
    sample_token_list = list(bevformer_results["results"].keys())
    for id in range(370, 400):
        render_sample_data(
            sample_token_list[id], pred_data=bevformer_results, out_path=sample_token_list[id]
        )
