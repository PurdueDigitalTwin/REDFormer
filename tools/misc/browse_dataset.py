...
def parse_args():
    ...
    parser.add_argument("config", help="train.sh config file path")
    parser.add_argument(
        "--skip-type",
        type=str,
        nargs="+",
...
def build_data_cfg(config_path, skip_type, cfg_options):
    ...
    # extract inner dataset of `RepeatDataset` as `cfg.data.train.sh`
    ...
    train_data_cfg["pipeline"] = [x for x in cfg.eval_pipeline if x["type"] not in skip_type]
