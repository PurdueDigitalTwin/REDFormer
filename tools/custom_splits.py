...
def create_splits_logs(split: str, nusc: "NuScenes") -> List[str]:
    ...
    if split in {"train", "val", "train_detect", "train_track"}:
        assert version.endswith(
            "trainval"
        ), "Requested split {} which is not compatible with NuScenes version {}".format(
            split, version
        )
    ...
    scene_to_log = {
        scene["name"]: nusc.get("log", scene["log_token"])["logfile"] for scene in nusc.scene
    }
    logs = set()
...
def create_splits_scenes(verbose: bool = False) -> Dict[str, List[str]]:
    """Similar to create_splits_logs, but returns a mapping from split to scene names, rather than
    log names. The splits are as follows:
    ...
    scene_splits = {
        ...
        "customrainval": customrainval,
        "customnightval": customnightval,
    }
