...
def create_kitti_info_file(data_path, pkl_prefix="kitti", save_path=None, relative_path=True):
    ...
    train_img_ids = _read_imageset_file(str(imageset_folder / "train.sh.txt"))
    ...
    print(f"Kitti info train.sh file is saved to {filename}")
...
def create_waymo_info_file(
    ...
    _calculate_num_points_in_gt(
        data_path, waymo_infos_train, relative_path, num_features=6, remove_outside=False
    )
    filename = save_path / f"{pkl_prefix}_infos_train.pkl"
    print(f"Waymo info train.sh file is saved to {filename}")
...
def _create_reduced_point_cloud(
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    kitti_infos = mmcv.load(info_path)
    cat2Ids = [
        dict(id=kitti_categories.index(cat_name), name=cat_name) for cat_name in kitti_categories
    ]
    coco_ann_id = 0
...
def get_2d_boxes(info, occluded, mono3d=True):
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        loc = loc + dim * (dst - src)
        offset = (info["calib"]["P2"][0, 3] - info["calib"]["P0"][0, 3]) / info["calib"]["P2"][
            0, 0
        ]
        loc_3d = np.copy(loc)
        loc_3d[0, 0] += offset
        ...
        corners_3d = box_np_ops.center_to_corner_box3d(
            gt_bbox_3d[:, :3], gt_bbox_3d[:, 3:6], gt_bbox_3d[:, 6], [0.5, 0.5, 0.5], axis=1
        )
        ...
        repro_rec = generate_record(
            ann_rec, min_x, min_y, max_x, max_y, sample_data_token, info["image"]["image_path"]
        )

        # If mono3d=True, add 3D annotations in camera coordinates
        if mono3d and (repro_rec is not None):
            repro_rec["bbox_cam3d"] = (
                np.concatenate([loc_3d, dim, rot], axis=1).astype(np.float32).squeeze().tolist()
            )
            repro_rec["velo_cam3d"] = -1  # no velocity in KITTI

            center3d = np.array(loc).reshape([1, 3])
            center2d = box_np_ops.points_cam2img(center3d, camera_intrinsic, with_depth=True)
