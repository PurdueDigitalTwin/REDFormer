...
def create_indoor_info_file(
    ...
    if pkl_prefix in ["sunrgbd", "scannet"]:
        ...
        if pkl_prefix == "sunrgbd":
            # SUN RGB-D has a train.sh-val split
            train_dataset = SUNRGBDData(root_path=data_path, split="train.sh", use_v1=use_v1)
            val_dataset = SUNRGBDData(root_path=data_path, split="val", use_v1=use_v1)
        else:
            # ScanNet has a train.sh-val-test split
            train_dataset = ScanNetData(root_path=data_path, split="train.sh")
        ...
        print(f"{pkl_prefix} info train.sh file is saved to {train_filename}")
    ...
    if pkl_prefix == "scannet":
        ...
        # S3DIS doesn't have a fixed train.sh-val split
