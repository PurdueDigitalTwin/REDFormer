...
class S3DISData:
    ...
    def __init__(self, root_path, split="Area_1"):
        ...
        self.sample_id_list = os.listdir(osp.join(self.data_dir, split))  # conferenceRoom_1
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_filename = osp.join(
                self.root_dir, "s3dis_data", f"{self.split}_{sample_idx}_point.npy"
            )
            pts_instance_mask_path = osp.join(
                self.root_dir, "s3dis_data", f"{self.split}_{sample_idx}_ins_label.npy"
            )
            pts_semantic_mask_path = osp.join(
                self.root_dir, "s3dis_data", f"{self.split}_{sample_idx}_sem_label.npy"
            )
            ...
            pts_instance_mask.tofile(
                osp.join(self.root_dir, "instance_mask", f"{self.split}_{sample_idx}.bin")
            )
            pts_semantic_mask.tofile(
                osp.join(self.root_dir, "semantic_mask", f"{self.split}_{sample_idx}.bin")
            )
...
class S3DISSegData:
    ...
    Args:
        ...
        split (str): Set split type of the data. Default: 'train.sh'.
    ...
    def __init__(
        self, data_root, ann_file, split="Area_1", num_points=4096, label_weight_func=None
    ):
    ...
    def get_scene_idxs_and_label_weight(self):
        r"""Compute scene_idxs for data sampling and label weight for loss \ calculation.

        We sample more times for scenes with more points. Label_weight is inversely proportional to
        number of class points.
        """
