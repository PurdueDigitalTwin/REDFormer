...
def get_image_path(
    ...
    return get_kitti_info_path(
        idx, prefix, info_type, ".png", training, relative_path, exist_check, use_prefix_id
    )
...
def get_label_path(
    ...
    return get_kitti_info_path(
        idx, prefix, info_type, ".txt", training, relative_path, exist_check, use_prefix_id
    )


def get_velodyne_path(
    idx, prefix, training=True, relative_path=True, exist_check=True, use_prefix_id=False
):
    return get_kitti_info_path(
        idx, prefix, "velodyne", ".bin", training, relative_path, exist_check, use_prefix_id
    )
...
def get_calib_path(
    ...
    return get_kitti_info_path(
        idx, prefix, "calib", ".txt", training, relative_path, exist_check, use_prefix_id
    )
...
def get_label_anno(label_path):
    ...
    annotations["bbox"] = np.array([[float(info) for info in x[4:8]] for x in content]).reshape(
        -1, 4
    )
...
def get_kitti_image_info(
    ...
    def map_func(idx):
        ...
        if calib:
            ...
            Tr_velo_to_cam = np.array([float(info) for info in lines[5].split(" ")[1:13]]).reshape(
                [3, 4]
            )
            Tr_imu_to_velo = np.array([float(info) for info in lines[6].split(" ")[1:13]]).reshape(
                [3, 4]
            )
            if extend_matrix:
...
def get_waymo_image_info(
    ...
    def map_func(idx):
        ...
        if velodyne:
            ...
            points = np.fromfile(Path(path) / pc_info["velodyne_path"], dtype=np.float32)
        ...
        image_info["image_path"] = get_image_path(
            idx, path, training, relative_path, info_type="image_0", use_prefix_id=True
        )
        if with_imageshape:
            img_path = image_info["image_path"]
            if relative_path:
        ...
        if label_info:
            label_path = get_label_path(
                idx, path, training, relative_path, info_type="label_all", use_prefix_id=True
            )
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path)
        ...
        if calib:
            ...
            Tr_velo_to_cam = np.array([float(info) for info in lines[6].split(" ")[1:13]]).reshape(
                [3, 4]
            )
            if extend_matrix:
                Tr_velo_to_cam = _extend_matrix(Tr_velo_to_cam)
            ...
            info["calib"] = calib_info
        if pose:
            pose_path = get_pose_path(idx, path, training, relative_path=False, use_prefix_id=True)
        ...
        while len(sweeps) < max_sweeps:
            ...
            prev_info["velodyne_path"] = get_velodyne_path(
                prev_idx, path, training, relative_path, exist_check=False, use_prefix_id=True
            )
            ...
            if if_prev_exists:
                ...
                prev_pose_path = get_pose_path(
                    prev_idx, path, training, relative_path=False, use_prefix_id=True
                )
