...
class SUNRGBDInstance:
    def __init__(self, line):
        ...
        self.box3d = np.concatenate(
            [self.centroid, np.array([self.l * 2, self.w * 2, self.h * 2, self.heading_angle])]
        )
...
class SUNRGBDData:
    ...
    def __init__(self, root_path, split="train.sh", use_v1=False):
            ...
            if has_label:
                ...
                if annotations["gt_num"] != 0:
                    ...
                    annotations["class"] = np.array(
                        [
                            self.cat2label[obj.classname]
                            for obj in obj_list
                            if obj.classname in self.cat2label.keys()
                        ]
                    )
                    annotations["gt_boxes_upright_depth"] = np.stack(
                        [obj.box3d for obj in obj_list if obj.classname in self.cat2label.keys()],
                        axis=0,
                    )  # (K,8)
