...
def create_lyft_infos(root_path, info_prefix, version="v1.01-train.sh", max_sweeps=10):
    ...
    available_vers = ["v1.01-train.sh", "v1.01-test"]
    assert version in available_vers
    if version == "v1.01-train.sh":
        train_scenes = mmcv.list_from_file("data/lyft/train.sh.txt")
    ...
    if test:
        ...
        print(
            f"train.sh scene: {len(train_scenes)}, \
    ...
    if test:
        ...
        print(
            f"train.sh sample: {len(train_lyft_infos)}, \
...
def export_2d_annotation(root_path, info_path, version):
    ...
    # info_2d_list = []
    cat2Ids = [
        dict(id=lyft_categories.index(cat_name), name=cat_name) for cat_name in lyft_categories
    ]
    coco_ann_id = 0
    ...
    for info in mmcv.track_iter_progress(lyft_infos):
        for cam in camera_types:
            ...
            coco_infos = get_2d_boxes(
                lyft, cam_info["sample_data_token"], visibilities=["", "1", "2", "3", "4"]
            )
