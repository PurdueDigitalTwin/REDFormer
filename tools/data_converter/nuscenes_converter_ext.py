...
from tools.data_converter.nuscenes_converter import (
...
def create_nuscenes_infos(
    ...
    if test:
        ...
        info_path = osp.join(out_path, f"{info_prefix}_infos_ext_test.pkl")
        mmcv.dump(data, info_path)
    else:
        print(f"train sample: {len(train_nusc_infos)}, val sample: {len(val_nusc_infos)}")
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(out_path, f"{info_prefix}_infos_ext_train.pkl")
        mmcv.dump(data, info_path)
        data["infos"] = val_nusc_infos
        info_val_path = osp.join(out_path, f"{info_prefix}_infos_ext_val.pkl")
        mmcv.dump(data, info_val_path)

    ## create rain and night data
    if info_prefix == "nuscenes":
        val_path = osp.join(out_path, f"{info_prefix}_infos_ext_val.pkl")
...
def _fill_trainval_infos(
    nusc, nusc_can_bus, train_scenes: set, val_scenes: set, test=False, max_sweeps=10
):
