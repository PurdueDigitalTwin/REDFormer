...
def parse_args():
    ...
    parser.add_argument(
        "--data-root", type=str, default="./data/nuimages", help="specify the root path of dataset"
    )
    parser.add_argument(
        "--version",
        type=str,
    ...
    parser.add_argument(
        "--nproc", type=int, default=4, required=False, help="workers to process semantic masks"
    )
...
def export_nuim_to_coco(nuim, data_root, out_dir, extra_tag, version, nproc):
    ...
    def process_img_anno(img_info):
        single_img_annos, max_cls_id = get_img_annos(
...
def main():
    ...
    for version in args.version:
        ...
        export_nuim_to_coco(
            nuim, args.data_root, args.out_dir, args.extra_tag, version, args.nproc
        )
