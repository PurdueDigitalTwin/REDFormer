...
from .utils.utils_radar import map_pointcloud_to_image
...
def binary_search(timestamp, can_bus_data, left, right):
    if left > right:
        return (
            left
            if abs(can_bus_data[left]["utime"] - timestamp)
            < abs(can_bus_data[right]["utime"] - timestamp)
            else right
        )

    mid = (left + right) // 2
    mid_timestamp = can_bus_data[mid]["utime"]

    if mid_timestamp == timestamp:
        return mid
    elif mid_timestamp < timestamp:
        return binary_search(timestamp, can_bus_data, mid + 1, right)
    else:
        return binary_search(timestamp, can_bus_data, left, mid - 1)


def find_closest_can_bus_data(timestamp, can_bus_data):
    index = binary_search(timestamp, can_bus_data, 0, len(can_bus_data) - 1)
    return can_bus_data[index]


def create_nuscenes_infos(
    root_path, out_path, can_bus_root_path, info_prefix, version="v1.0-trainval", max_sweeps=10
):
    ...
    val_scenes = {available_scenes[available_scene_names.index(s)]["token"] for s in val_scenes}

    test = "test" in version
    if test:
        print(f"test scene: {len(train_scenes)}")
    else:
        print(f"train.sh scene: {len(train_scenes)}, val scene: {len(val_scenes)}")
    ...
    if test:
        ...
        print(
            "train.sh sample: {}, val sample: {}".format(
...
def _fill_trainval_infos(nusc, nusc_can_bus, train_scenes, val_scenes, test=False, max_sweeps=10):
    """Generate the train.sh/val infos from the raw data.
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        for cam in camera_types:
            ...
            _, fake_boxes, camera_intrinsic = nusc.get_sample_data(
            ...
            for radar_channel in RADARS_FOR_CAMERA[cam]:
                radar_pcs, _ = RadarPointCloud.from_file_multisweep(
                    nusc, sample, radar_channel, cam, max_sweeps
                )
                all_radar_pcs.points = np.hstack((all_radar_pcs.points, radar_pcs.points))

            # image information in COCO format
            pc_rd = np.array(all_radar_pcs.points.tolist())
            cam_intrinsic = np.array(calib.tolist())[:, :3]

            points_on_images, coloring, _ = map_pointcloud_to_image(pc_rd, cam_intrinsic)

            cam_info.update(radarpts_image=points_on_images)

            ##TODO: ADD VELOCITY INFO into pkl file
            scene_name = nusc.get("scene", sample["scene_token"])["name"]
            vehicle_monitor_path = (
                "/media/can/samsungssd/nuscenes/full/can_bus/"
                + scene_name
                + "_vehicle_monitor.json"
            )

            if os.path.exists(vehicle_monitor_path):
                with open(vehicle_monitor_path) as f:
                    can_bus_data = json.load(f)
                if len(can_bus_data) == 0:
                    cam_info.update(ego_velo=0)
                else:
                    timestamp = sample["timestamp"]
                    closest_time = 100000
                    index = 0
                    for i in range(len(can_bus_data)):
                        if abs(timestamp - can_bus_data[i]["utime"]) < closest_time:
                            closest_time = abs(timestamp - can_bus_data[i]["utime"])
                            index = i
                        else:
                            continue
                    ego_velocity = can_bus_data[index]["vehicle_speed"]

                    #
                    # can_bus_data.sort(key=lambda x: x['utime'])
                    #
                    # timestamp = sample['timestamp']
                    # closest_can_bus_data = find_closest_can_bus_data(timestamp, can_bus_data)
                    # ego_velocity = closest_can_bus_data['vehicle_speed']
                    cam_info.update(ego_velo=ego_velocity)
            else:
                cam_info.update(ego_velo=0)
...
def export_2d_annotation(root_path, info_path, version, mono3d=True):
    ...
    # info_2d_list = []
    cat2Ids = [
        dict(id=nus_categories.index(cat_name), name=cat_name) for cat_name in nus_categories
    ]
    coco_ann_id = 0
    coco_2d_dict = dict(annotations=[], images=[], categories=cat2Ids, radar=[])
    ...
    for sample in nusc.sample:
        ...
        for sensor_name in sample["data"]:
            if sensor_name in camera_types:
                ...
                for radar_channel in RADARS_FOR_CAMERA[sensor_name]:
                    radar_pcs, _ = RadarPointCloud.from_file_multisweep(
                        nusc, sample, radar_channel, sensor_name, num_sweeps
                    )
                    all_radar_pcs.points = np.hstack((all_radar_pcs.points, radar_pcs.points))
                ...
                coco_2d_dict["radar"].append(
                    dict(
                        file_name=image_data["filename"],
                        points_on_image=points_on_images.tolist(),
                        rain=scene_rain,
                        night=scene_night,
                    )
                )
...
def post_process_coords(
    ...
    canvas, return None if no intersection.
...
def generate_record(
    ...
    """Generate one 2D annotation record given various informations on top of the 2D bounding box
    coordinates.
