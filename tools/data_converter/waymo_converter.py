...
class Waymo2KITTI:
    ...
    def save_image(self, frame, file_idx, frame_idx):
        ...
        for img in frame.images:
            img_path = (
                f"{self.image_save_dir}{str(img.name - 1)}/"
    ...
    def save_calib(self, frame, file_idx, frame_idx):
        ...
        for i in range(5):
            calib_context += (
                "Tr_velo_to_cam_" + str(i) + ": " + " ".join(Tr_velo_to_cams[i]) + "\n"
            )
    ...
    def save_lidar(self, frame, file_idx, frame_idx):
        ...
        # First return
        points_0, cp_points_0, intensity_0, elongation_0 = self.convert_range_image_to_point_cloud(
        ...
        # Second return
        points_1, cp_points_1, intensity_1, elongation_1 = self.convert_range_image_to_point_cloud(
        ...
        pc_path = (
            f"{self.point_cloud_save_dir}/{self.prefix}"
    ...
    def save_pose(self, frame, file_idx, frame_idx):
        ...
        np.savetxt(
            join(
                f"{self.pose_save_dir}/{self.prefix}"
                + f"{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.txt"
            ),
            pose,
        )
    ...
    def convert_range_image_to_point_cloud(
        ...
        for c in calibrations:
            ...
            elongation_tensor = tf.gather_nd(
                range_image_tensor[..., 2], tf.where(range_image_mask)
            )
