...
class RadarPointCloudWithVelocity(RadarPointCloud):
    ...
    def rotate_velocity(cls, pointcloud, transform_matrix):
        ...
        ## TODO: Only want x,y,z and velocity
        # return np.delete(pointcloud,[3,4,5,6,7],0)
    ...
    def from_file_multisweep(
        ...
        """Return a point cloud that aggregates multiple sweeps. As every sweep is in a different
        coordinate frame, we need to map the coordinates to a single reference frame. As every
        sweep has a different timestamp, we need to account for that in the transformations and
        timestamps.
        ...
        for _ in range(nsweeps):
            ...
            velocity_trans_matrix = reduce(
                np.dot,
                [ref_from_car_rot, car_from_global_rot, global_from_car_rot, car_from_current_rot],
            )
...
def generate_pc_hm(output, pc_dep, calib, opt):
    ...
    for i, [pc_dep_b, bboxes_b, depth_b, dim_b, rot_b] in enumerate(
        ...
        for j, [bbox, depth, dim, alpha] in enumerate(zip(bboxes_b, depth_b, dim_b, alpha_b)):
            ...
            ct = torch.tensor(
                [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], device=pc_dep_b.device
            )
...
def pc_dep_to_hm_torch(pc_hm, pc_dep, dep, bbox, dist_thresh, opt):
    ...
    bbox_int = torch.tensor(
        [torch.floor(bbox[0]), torch.floor(bbox[1]), torch.ceil(bbox[2]), torch.ceil(bbox[3])],
        dtype=torch.int32,
    )  # format: xyxy
    ...
    if len(nonzero_inds) and len(nonzero_inds[0]) > 0:
        ...
        if len(pc_dep_match) > 0:
            ...
            pc_hm[opt.pc_feat_channels["pc_dep"], h_min : h_max + 1, w_min : w_max + 1 + 1] = dist
...
def pc_dep_to_hm(pc_hm, pc_dep, dep, bbox, dist_thresh, opt):
    ...
    bbox_int = np.array(
        [np.floor(bbox[0]), np.floor(bbox[1]), np.ceil(bbox[2]), np.ceil(bbox[3])], np.int32
    )  # format: xyxy
