...
def custom_encode_mask_results(mask_results):
    ...
    for i in range(len(cls_segms)):
        encoded_mask_results.append(
            mask_util.encode(np.array(cls_segms[i][:, :, np.newaxis], order="F", dtype="uint8"))[0]
        )  # encoded with RLE
...
def collect_results_cpu(result_part, size, tmpdir=None):
    ...
    if rank != 0:
        ...
        # sort the results
        ordered_results = []
        """bacause we change the sample of the evaluation stage to make sure that each gpu will
        handle continuous sample,"""
