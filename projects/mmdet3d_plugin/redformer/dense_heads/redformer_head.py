...
class RadarBackbone(nn.Module):
    ...
    __slots__ = ("max_num_radar_per_grid", "embedding_dim",
                 "_embedding", "_gated_unit", "_mha")
    ...
    def __init__(self, max_num_radar_per_grid: int, hidden_size: int) -> None:
        ...
        self._embedding = nn.Embedding(num_embeddings=self.max_num_radar_per_grid,
    ...
    def forward(self, input: torch.Tensor) -> torch.FloatTensor:
        ...
        assert hidden.shape == (width * length, self.embedding_dim)
        ...
        #
        # ref_2d = BEVFormerEncoder.get_reference_points(H=length, W=width, dim='2d')
        # deforma_output = self.defomable_selfatten.forward(query=hidden,
        #                                                   key=hidden,
        #                                                   value=hidden,
        #                                                   reference_points=ref_2d,
        #                                                   spatial_shapes=torch.tensor([[length, width]],
        #                                                                               device=hidden.device),
        #                                                   level_start_index=torch.tensor([0], device=hidden.device),
        #                                                   )
        # output = hidden + deforma_output

        output = hidden

        # mha_output, _ = self._mha.forward(query=hidden, key=hidden, value=hidden)
        # output = hidden + mha_output

        # output reshape
        output = output.squeeze(0)
        output = output.view(width, length, -1)
        # output = hidden.view(width, length, -1)
...
class REDFormerHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(REDFormerHead, self).__init__(
        ...
        # )
        self.radar_emb = RadarBackbone(max_num_radar_per_grid=10, hidden_size=256)
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None, only_bev=False, radar_hist=None):
        """Forward function.
        Args:
        ...
        # self.radar_backbone.weight.to(dtype)
        # self.radar_backbone.bias.to(dtype)
        embed_dim = bev_queries.shape[1]

        if radar_hist is not None:
            # radar_hist = torch.unsqueeze(radar_hist, dim=0)

            radar_hist = radar_hist.long()  # NOTE: for embedding only
            radar_bev = self.radar_emb(radar_hist).view(-1, 256)
            # radar_bev = self.radar_backbone(radar_hist)
            # radar_bev = radar_bev.reshape(embed_dim, -1).transpose(0, 1)
