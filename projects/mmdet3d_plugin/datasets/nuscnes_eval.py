...
from nuscenes.eval.common.utils import Quaternion, boxes_to_sensor, quaternion_yaw
...
from tools.custom_splits import create_splits_scenes
...
def center_in_image(
    box, intrinsic: np.ndarray, imsize: Tuple[int, int], vis_level: int = BoxVisibility.ANY
) -> bool:
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(
        ...
        :param eval_set: The dataset split to evaluate on, e.g. train.sh, val or test.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="val",
        help="Which dataset split to evaluate on, train.sh, val or test.",
    )
    parser.add_argument(
        "--dataroot", type=str, default="data/nuscenes", help="Default nuScenes data directory."
    )
    parser.add_argument(
        "--version",
        type=str,
    ...
    parser.add_argument(
        "--render_curves", type=int, default=1, help="Whether to render PR and TP curves to disk."
    )
    parser.add_argument("--verbose", type=int, default=1, help="Whether to print to stdout.")
    args = parser.parse_args()
    # args.eval_set = 'customrainval'
