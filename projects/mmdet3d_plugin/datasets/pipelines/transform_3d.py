        ...
        if results['radar_pts']:
            ...
            for batch_num in range(img_shape[3]):
                for x, y in zip(radar_pts[batch_num][0], radar_pts[batch_num][1]):
                    img[round(y), round(x), 0, batch_num] = min((img[round(y), round(x), 0, batch_num] + 85), 255)
                    img[round(y), round(x), 1, batch_num] = min((img[round(y), round(x), 1, batch_num] + 85), 255)
                    img[round(y), round(x), 2, batch_num] = min((img[round(y), round(x), 2, batch_num] + 85), 255)
...
@PIPELINES.register_module()
class RadarPoints2BEVHistogram(object):
    def __init__(self, pc_range, bev_h, bev_w):
        self.bev_w = bev_w
        self.bev_h = bev_h
        self.xmin, self.xmax, self.ymin, self.ymax = pc_range[0], pc_range[3], pc_range[1], pc_range[4]

    def __call__(self, results):
        radar_points = results['radar_points']
        radar_hist = np.histogram2d(
            x=radar_points[:, 0],
            y=radar_points[:, 1],
            bins=[self.bev_w, self.bev_h],
            range=[[self.xmin, self.xmax], [self.ymin, self.ymax]]
        )[0].astype(radar_points.dtype)
        results['radar_hist'] = radar_hist
        return results
